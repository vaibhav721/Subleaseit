...
const MapModal = ({ show, handleClose, google, address }) => {
        ...
        service.getDistanceMatrix(
          ...
          (response, status) => {
            if (status === google.maps.DistanceMatrixStatus.OK) {
              const originalDistance = response.rows[0].elements[0].distance.value; // Assuming distance is in meters
              const reducedDistanceInMeters = Math.max(originalDistance - 200, 0); // Subtract 500 meters and ensure the value is not negative
              const reducedDistanceInKilometers = reducedDistanceInMeters / 1000; // Convert meters to kilometers
              const reducedDistanceWithOneDecimal = reducedDistanceInKilometers.toFixed(1); // Limit the decimal places to 1
              const distanceText =
              reducedDistanceWithOneDecimal + " km to USC";
  ...
  const markerIcon = {
    url: process.env.PUBLIC_URL + "/assets/img/subleaseit/white_background.jpeg",
    ...
    <Modal show={show} onHide={handleClose}>
      <div className="map-container">
        {isMapReady ? (
          ...
          >
            <div>
              <span style={{ color: '#ed8887', marginRight: '5px' }}>■</span> DPS Zone (Safe Zone)
            </div>
            <div>
              <Marker
